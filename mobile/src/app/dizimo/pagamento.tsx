...
import { ArrowLeft, Church, Clock, Copy, Loader, RotateCcw } from "lucide-react-native";
...
export default function PagamentoPixScreen() {
  ...
  const { valor, mes, ano } = useLocalSearchParams<{ valor: string; mes: string; ano: string }>();
  ...
  const criarCobranca = useCallback(() => {
    ...
    criarCobrancaPix({ tipo: "dizimo", valor: valorNum, mes: mesNum, ano: anoNum }).then(
      (resultado) => {
        if (!ativo) return;
        if (!resultado.ok) {
  ...
  useEffect(() => {
    ...
    const intervalo = setInterval(async () => {
      ...
      if (status === "aprovado") {
        ...
        router.replace({
          pathname: "/dizimo/sucesso",
          params: {
            valor: String(valorNum),
            mes: String(mesNum),
            dataPagamento: formatarDataHora(new Date()),
          },
        });
  ...
  }, [cobranca, router, valorNum, mesNum]);
    ...
    <SafeAreaView className="flex-1 bg-background" edges={["top"]}>
      <View className="flex-row items-center px-6 py-4">
        ...
        <Text className="flex-1 text-center text-lg font-bold text-foreground">
          Dízimo de {MESES[mesNum - 1]?.toLowerCase()}
        </Text>
        <View className="w-8" />
      </View>
      ...
      >
        ...
        {carregando ? (
          ...
          <>
            ...
            <Text className="text-center text-sm text-muted-foreground">
              Pode fechar o app depois de pagar — a confirmação chega pelo banco, não por esta
              tela. O código vale por 30 minutos.
            </Text>
        ...
        <View className="mt-2 w-full gap-3 border-t border-muted pt-4">
          <View className="flex-row items-start gap-3">
            <Church size={18} strokeWidth={1.75} color={corIcone.mutedForeground} />
            <Text className="flex-1 text-sm text-muted-foreground">
              Prefere entregar em mãos? A secretaria lança o seu dízimo do mesmo jeito, e ele
              aparece aqui como pago.
            </Text>
          </View>
        </View>
      </ScrollView>
